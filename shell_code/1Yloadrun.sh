...
function prepare_conf()
{
    ...
    if [ "$1" = "all" ];then
       ...
       echo "cp -P $conf_dir/sfx_aerospike/1Y/$1_$default_conf $conf_dir/$default_conf"
       cp -P $conf_dir/sfx_aerospike/1Y/$1_$default_conf $conf_dir/$default_conf
    fi
}
...
while getopts :a:b:l:n:m:d:h:c:w:t:r:o:s:g: vname 
do
    case "$vname" in
       ...
       h) echo "-a for phrase: load | run" 
          ...
          echo "-g: will use larger recordcount like 1Yi "
          echo "-o: YCSB Options: "
          ;;
...
function doload()
{
    ...
    echo iostat -txdm /dev/$io_sfx 1 >> $iostat_log &
    iostat -txdm /dev/$io_sfx 1 >> $iostat_log &
    
    echo $! > $iostat_pid

    outfile=$running.$device.result
    ...
    if [ "$ret" = "n" ];then
        ...
        bin/ycsb load aerospike -P workloads/workloada -p as.host=localhost -p as.namespace=$namespace -threads $thread -p recordcount=$recount -s >> $outfile
    elif [ "$ret" = "y" ];then
       echo "workloada....."
       ./bin/ycsb load aerospike -s -P workloads/workloada -p as.timeout=5000 >> $outfile
    fi
...
if [ "$action" = "load" ];then
    if [ "$device" = "all" ];then
        ...
        do
            ...
            if [ "$1" = "ext4" ];then
                tune2fs -O has_journal /dev/sfd0n1
                mount /dev/${sfxd[0]} /opt/aerospike/data/
            fi
            if [ "$1" = "xfs" ];then
                mount /dev/${sfxd[1]} /opt/aerospike/xfsdata/ 
            fi
        ...
        cls_data $device
        doload $running.$device.result
    fi
elif [ "$action" = "run" ];then
    echo -e "$action phrase starts at:  "$logctime "\n"  > $outfile
    ./bin/ycsb run aerospike -s -P workloads/workloada >> $outfile
    cendtime=$flag`date +%d%m%y_%H:%M:%S`
    echo -e "\n$action phrase ends at: "$cendtime "\n"  >> $outfile &
else
