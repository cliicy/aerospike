...
#    service aerospike status
    ...
    if [ "$ret" = "n" ];then
        ...
        #bin/ycsb load aerospike -P workloads/workloada -p as.host=localhost -p as.namespace=$2 -threads 10 -p recordcount=200000000 -s >./outputload/$3_load.txt
	...
	for((i=16;i<=112;i+=16))
        ...
        service aerospike restart
        service aerospike status
        bin/ycsb load aerospike -P workloads/workloada -p as.host=localhost -p as.namespace=$2 -threads $i -p recordcount=50000000 -s >./outputload/$i"_load.txt"
        done
